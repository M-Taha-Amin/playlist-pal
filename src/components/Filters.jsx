...
const Filters = props => {
  const { setWatched, dataSize } = props;
  ...
  const handleFilterApply = e => {
    ...
    if (watchedVideosVal > 0 && watchedVideosVal <= dataSize) {
      setWatched(parseInt(watchedVideosRef.current.value));
    } else {
      alert(
        `Watched Videos Value can only be from 1 to Total Number of Playlist Videos (${dataSize})`
      );
    }
  };
